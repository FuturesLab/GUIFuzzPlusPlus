...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
                       "+a:Ab:B:c:CdDe:E:f:F:g:G:hi:J:K:I:l:L:m:M:nNo:Op:P:QRs:S:t:"
                       "T:UV:WXx:YzZ")) > 0) {
    ...
    switch (opt) {
      ...
      case 'K':
         ...
         afl->fsrv.gui_dir = ck_strdup(optarg);
         break;

      case 'J':
      // this is the shadowqueue case
         int value = strtol(optarg, NULL, 10);
         afl->shadowqueue = value;
         if (afl->shadowqueue) {
         WARNF(
         "Using shadowqueue, AFL will not use the queue.");}
         break;
