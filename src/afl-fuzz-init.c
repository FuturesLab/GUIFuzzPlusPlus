...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  /* === BEGIN SHADOWQUEUE COPY === */
  if (afl->shadowqueue) {
  
    DIR *d = opendir(orig_q);
    struct dirent *d_ent;

    if (d) {
      u8 *shadow_dir = alloc_printf("%s/shadowqueue", afl->out_dir);
      mkdir(shadow_dir, 0700);  // Create if not exists

      while ((d_ent = readdir(d))) {

        if (d_ent->d_name[0] != '.') {

          u8 *src_name = alloc_printf("%s/%s", orig_q, d_ent->d_name);
          u8 *dst_name = alloc_printf("%s/%s", shadow_dir, d_ent->d_name);

          int counter = 0;
          while (access(dst_name, F_OK) == 0) {
            ck_free(dst_name);
            counter++;
            dst_name = alloc_printf("%s/%s_copy%d", shadow_dir, d_ent->d_name, counter);
          }
