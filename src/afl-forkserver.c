...
static void afl_fauxsrv_execv(afl_forkserver_t *fsrv, char **argv) {
  ...
  while (1) {
    ...
    if (!child_pid) {  // New child
      ...
      pid_t outer_child_pid = getpid(); // Get the current PID of this soon to be GUI process
      python_pid = fork();

      if (!python_pid) {
        printf("Non-forkserver exec'ing, with pid = %ld\n", (long) outer_child_pid);
        char child_pid_str[16];
        sprintf(child_pid_str, "%d", (int)outer_child_pid); // Convert pid_t to a string
        
        char *pargs[] = {"/usr/bin/python3", fsrv->gui_dir, "-o", fsrv->out_file, "-p", child_pid_str, NULL};
...
fsrv_run_result_t __attribute__((hot)) afl_fsrv_run_target(
  ...
  if (!fsrv->use_fauxsrv)
  {
    pid_t outer_child_pid = fsrv->child_pid; // Get the current PID of this soon to be GUI process
  ...
  if (unlikely(
          ...
          (WIFSIGNALED(fsrv->child_status) && WTERMSIG(fsrv->child_status) != SIGINT && WTERMSIG(fsrv->child_status) != SIGTERM) ||
    ...
    printf("Crashed with Exit code: %d\n", WTERMSIG(fsrv->child_status));
