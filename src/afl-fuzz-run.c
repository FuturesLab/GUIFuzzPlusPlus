...
// === BEGIN UDP RUNTIME PING ===
int sock = socket(AF_INET, SOCK_DGRAM, 0);
if (sock >= 0) {
    struct sockaddr_in server_addr;
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(50000);  // 🔧 Your host-side listener port
    inet_pton(AF_INET, "155.98.69.215", &server_addr.sin_addr);  // 🔧 Host IP from VM's POV

    //u64 now = get_cur_time();  // ms
    //u64 runtime_sec = (now - afl->start_time) / 1000;
    // this is the total runtime maybe??
    u64 runtime_sec = (get_cur_time() + afl->prev_run_time - afl->start_time) / 1000;
    char message[32];
    snprintf(message, sizeof(message), "%llu", runtime_sec);
